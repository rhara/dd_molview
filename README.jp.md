...
経由でPythonインタプリタを埋め込み、`dd_viewer` の既存の、無改変のPython
パッケージと、自前で取り込んだ `dd_cview_core` モジュール（複数受容体/
リガンドのコレクション・配列抽出/HTML生成・ダッシュボードテーブル——
`dd_molview` が廃止された際にそのコアロジックを無改変のまま吸収したもの）
をそのまま呼び出す。境界は1つの狭いJSON入出力モジュール
...
そして `dd_viewer` がインストールされたPython環境（`dd_cview_core` は
`python/` 以下にあり、別途インストール不要——後述）。
...
のは `dd_viewer` 自体と、`dd_cview_backend.py`/`dd_cview_core` が実際に
importするその依存のサブセット（RDKit、biopython、pandas、numpy、
py3Dmol）、そして `pybind11` だけ——`dd_cview` の埋め込みバックエンドが
一切触れない `dd_viewer` の追加GUI/Web依存（Streamlit）は不要:
...
# dd_viewer自体はconda-forgeに無いので、チェックアウトから
# editableインストールする——上のconda依存が dd_cview が実際にimportする
# ものを全てカバーしているので --no-deps で良い。dd_cview_core
# （python/dd_cview_core/ 参照）はこのプロジェクト自前のモジュールで、
# インストール不要——python/ が実行時にsys.pathへ直接追加される。
pip install --no-deps -e ../dd_viewer
```

`dd_cview_core` 自身のpytestスイート（`python/tests/`——`dd_molview`の
コアロジックを吸収した際にそのテストスイートも移植したもの）は、C++
ビルドなしで単体実行できる:
...
（Windowsでは`...\python.exe`）で、`dd_viewer` が
...
- **PYTHONHOMEは明示的に設定し、`sys.path`のcwdエントリは除去する。**
  `dd_viewer` と `dd_cview` は同じ親ディレクトリ
